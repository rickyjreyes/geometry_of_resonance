...
# Time parameters
dt = 0.01
num_steps = 1000  # Extended time to allow well formation
c = 1.0
lambda_factor = 0.2  # Increased nonlinearity for stronger confinement
...
for t in range(num_steps):
    cx1, cx2 = -r * np.cos(omega * t), r * np.cos(omega * t)
    cy3 = -r * np.sin(omega * t)

    wave1 = np.exp(-((X - cx1)**2 + Y**2)) * np.cos(3 * (X - cx1) + phi1)
    wave2 = np.exp(-((X - cx2)**2 + Y**2)) * np.cos(3 * (X - cx2) + phi2)
    wave3 = np.exp(-(X**2 + (Y - cy3)**2)) * np.cos(3 * (Y - cy3) + phi3)
    ...
    curvature += 0.01 * info_map  # symbolic information feedback

    # Add a confinement feedback potential based on accumulated mass
    ...
    fig.suptitle(f"Step {t} | Entropy: {entropy:.3f} | Stable Masses: {num_masses}", fontsize=14)
    plt.pause(0.01)
