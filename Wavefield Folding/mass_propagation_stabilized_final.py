...
def compute_temporal_curvature(psi_history, dt):
    if len(psi_history) < 3:
        return np.zeros_like(psi_history[-1])
    psi_t_minus = psi_history[-3]
    psi_t = psi_history[-2]
    psi_t_plus = psi_history[-1]
    d2psi_dt2 = (psi_t_plus - 2 * psi_t + psi_t_minus) / dt**2
    with np.errstate(divide='ignore', invalid='ignore'):
        T_psi = -d2psi_dt2 / psi_t
        T_psi[~np.isfinite(T_psi)] = 0
    return T_psi

def compute_effective_metric(psi, dx, kappa=0.1):
    grad_x, grad_y = np.gradient(psi, dx)
    with np.errstate(divide='ignore', invalid='ignore'):
        metric_trace = 2 + kappa * (grad_x**2 + grad_y**2) / (np.abs(psi)**2 + 1e-10)
    metric_trace[~np.isfinite(metric_trace)] = 0
    return metric_trace

def curvature_tensor_feedback(laplacian, psi):
    with np.errstate(divide='ignore', invalid='ignore'):
        tensor_term = (laplacian**2) / (psi**2 + 1e-10)
        tensor_term[~np.isfinite(tensor_term)] = 0
    return tensor_term

psi_history = deque(maxlen=3)
...
# Phase setup
phi_list = np.linspace(0, 2 * np.pi, 7) + 0.1 * np.random.rand(7)
r = 3.0
omega = 0.2
...
# Constants
gamma, delta = 0.08, 0.0005
alpha_confinement = 0.3
info_map = np.random.uniform(0.5, 1.5, size=psi.shape)
info_map += 0.05 * np.random.normal(size=psi.shape)
# Adaptive threshold based on average curvature over time
if curvature_log:
    avg_curv = np.mean(curvature_log[-50:])
    stabilization_threshold = 0.3 + 0.015 * np.tanh(avg_curv / 150)
else:
...
for t in range(num_steps):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        curvature_raw = laplacian / (psi + 1e-3 * np.exp(-0.5 * psi**2))
        curvature_raw[~np.isfinite(curvature_raw)] = 0
        curvature = -np.clip(curvature_raw, -1000, 1000)

    curvature = gaussian_filter(curvature, sigma=1.0)
    ...
    repulsion = -0.005 * (mass_accum > 2.0).astype(float)
    mass_kernel = gaussian_filter(mass_accum, sigma=1)
    local_pressure = 0.01 * (mass_kernel - mass_accum)
    ...
    if len(psi_history) >= 3:
        T_psi = compute_temporal_curvature(psi_history, dt)
        geff_trace = compute_effective_metric(psi, dx)
        curv_tensor_term = curvature_tensor_feedback(laplacian, psi)
    else:
        T_psi = np.zeros_like(psi)
        geff_trace = np.zeros_like(psi)
        curv_tensor_term = np.zeros_like(psi)


    decay_factor = 0.9999
    psi_new = decay_factor * (2 * psi - psi_old + c**2 * dt**2 * (
        laplacian + lambda_factor * psi**3 + strong_conf + 0.02 * curvature - restoring_potential + weak_force - energy_damp + 0.001 * T_psi - 0.002 * geff_trace - 0.001 * curv_tensor_term))
