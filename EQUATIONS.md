...
S_eff[φ] = ∮_Γ w(s) (∂_s φ(s) − σ(s))² ds + Λ ( ∮_Γ ∂_s φ(s) ds − 2π n ).
...
∮_Γ w ∂_s φ ds = ∮_Γ w σ ds + 2π α ( ∮_Γ ds ) / ( ∮_Γ ds / w(s) ).
...
The CLE-series contains all previously missing equations that finalize the
curvature-locked ψ-electron derivation.
...
\[
F'_{\mathrm{fb}}(\sigma) = -2\sigma
\]

Derived from the variational collapse condition under \( \Theta[\psi] = \sigma(\psi) \).
...
\[
\nabla^{2}\psi
= \frac{1}{r}\,\partial_\theta\!\big(r\,\partial_\theta \psi\big)
+ \frac{1}{(R + r\cos\theta)^2}\, \partial_\phi^2 \psi
\]
...
For the electron: \( R \approx 386.3\ \mathrm{fm} \).
...
δ̈_γ + cₛ²(t) ⋅ k² ⋅ δ_γ = −k² ⋅ Φ
δ̈_b + ℛ(t) ⋅ cₛ²(t) ⋅ k² ⋅ δ_γ = −k² ⋅ Φ
ℛ(t) = E_comp / E_rad

CM6 — Sound Speed from Curvature Feedback
cₛ²(t) = [1 / (3(1 + ℛ(t)))] ⋅ [1 − β_curv ⋅ E_curv(t) / E_tot]

CM7 — Curvature Diffusion (Silk Analog)
∂ₜ δ_γ → ∂ₜ δ_γ ⋅ D_curv(t) ⋅ k² ⋅ δ_γ
D_curv(t) = ⟨|∇ψ|²⟩ / ⟨|ψ|²⟩
...
cₛ(t) = sqrt( ∂P_curv / ∂ρ_curv )

CM20 — Θ-Based Expansion Law
H(t) = ȧ_WCT / a_WCT = sqrt( ρ_Θ(t) / 3|K| )
```
